...
const HomePage = () => {
  ...
  const fetchData = async() => {
    try {
      const [igFeedData, homePageData] = await Promise.all([GetBTBInstagramFeed(), GetConfig("general", {"type": "homepage"})]);
                  ...
                  return <div className="relative w-[200px] h-[200px] md:w-[240px] md:h-[240px] my-[25px] bg-cover bg-center rounded-[30px]">
                      ...
                      <div className="absolute inset-0 hover:bg-[#243F6D] hover:opacity-80 flex items-center justify-center rounded-[30px]">
                        <div className="text-white group">
                          <a href={val?.link || '#'} target="_blank" className="border-2 px-6 py-2 text-[15px] rounded-full opacity-0 font-semibold group-hover:border-1 group-hover:opacity-100 transition-opacity duration-300">
                            Learn More
                          </a>
                        </div>
                      </div>
