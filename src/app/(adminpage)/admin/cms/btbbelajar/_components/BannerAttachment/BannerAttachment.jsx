...
const BannerAttachment = ({datas, grade, onDelete, onChangeAttachment, addAttachment}) => {
  return (
    <div className="flex flex-col gap-2">
      {datas.length > 0 && datas.map((res, index) => (
        <div>
          {typeof res === 'string' ? (
            ...
            <div className="flex">
              ...
              <div className="w-full">
                <AttachInput filename={res?.name} onChange={(file) => onChangeAttachment(file, grade, index)} />
              </div>
            </div>
          )}
        </div>
      ...
      <Button className="mt-2 ml-9 w-36" onClick={() => addAttachment(grade)} size={"sm"}>
        Add
      </Button>
    </div>
  )
  ...
  return (
    <>
      ...
      <p className='text-sm text-gray-500'>
        Ukuran Maksimum 2MB. Format Gambar (.jpg)
      </p>
      <input
        type="file"
        ref={fileInputRef}
